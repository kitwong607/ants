...
class LongSeriesSignalTesterAPM5T(FutureAbstractStrategy):
    ...
    STRATEGY_NAME = "LongSeriesSignalTesterAPM5T"
    STRATEGY_SLUG = "LongSeriesSignalTesterAPM5T"
    VERSION = "1"
    LAST_UPDATE_DATE = "20190503"
    ...
    def Setup(self, session):
        ...
        '''
        signal = entry.XHigherKAMAY(self, "closeD", 1, "closeD", 30)
        #signal = entry.XHigherKAMAY(self, "close", 0, "close", 30)
        signal = entry.XHigherMaxPreviousY(self, "close", 0, "high", 36)
    ...
    def CalculateEntrySignal(self, bar):
        if self.nextSignalIdx < len(self.entrySignals):
            signal = self.entrySignals[self.nextSignalIdx]
            if signal.CalculateSignal(bar):
                self.nextSignalIdx += 1
                if self.entryLabel != "":
                    self.entryLabel += " "
                self.entryLabel += signal.Label()

                if self.session.mode == SessionMode.IB_LIVE or self.session.mode == SessionMode.IB_DALIY_BACKTEST or True:

                    self.Log(bar.adjustedTime,"Entry signal["+str(self.session.config.sid)+"-"+signal.Label()+"]: True")
            else:
                if self.session.mode == SessionMode.IB_LIVE or self.session.mode == SessionMode.IB_DALIY_BACKTEST or True:
                    pass
                    #self.Log("Entry signal["+str(self.session.config.sid)+"-"+signal.Label() + "]: False")

        #print(self.nextSignalIdx, len(self.entrySignals))
        if self.nextSignalIdx == len(self.entrySignals):
            self.Entry(bar.closePrice, bar.adjustedDate, bar.adjustedTime, OrderType.LIMIT, self.entryLabel, self.baseQuantity)
