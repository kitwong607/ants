...
class IBContract:
    HKEXFutureMonthCode = ["F","G","H","J","K","M","N","Q","U","V","X","Z"]
    """ Usually, the easiest way to define a Stock/CASH contract is through 
    these four attributes.  """

    @staticmethod
    def MHIFut(lastTradeDateOrContractMonth = None):
        #! [cashcontract]
        contract = Contract()
        contract.symbol = "MHI"
        ...
        if lastTradeDateOrContractMonth is None:
            ...
            elif len(lastTradeDateOrContractMonth)==3:
                contract.lastTradeDateOrContractMonth = "200" + str(lastTradeDateOrContractMonth)
            else:
                contract.lastTradeDateOrContractMonth = lastTradeDateOrContractMonth

        contract.code = contract.symbol + IBContract.HKEXFutureMonthCode[int(contract.lastTradeDateOrContractMonth[4:])-1] + contract.lastTradeDateOrContractMonth[2:4]

        #! [cashcontract]
        return contract

    @staticmethod
    def HSIFut(lastTradeDateOrContractMonth = None):
        #! [cashcontract]
        contract = Contract()
        contract.symbol = "HSI"
        contract.secType = "FUT"
