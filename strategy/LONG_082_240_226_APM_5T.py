...
class LONG_082_240_226_APM_5T(FutureAbstractStrategy):
    ...
    STRATEGY_NAME = "LONG_082_240_226_APM_5T"
    STRATEGY_SLUG = "LONG_082_240_226_APM_5T"
    VERSION = "1"
        ...
        for signal in self.entrySignals:
            if signal.CalculateSignal(bar):
                ...
                if label != "":
                    label += " "
                label += signal.Label()
                self.Log(LONG_082_240_226_APM_5T.STRATEGY_NAME, "Entry signal["+str(self.session.config.sid)+"-"+signal.Label()+"]: True")
            else:
                self.Log(LONG_082_240_226_APM_5T.STRATEGY_NAME, "Entry signal["+str(self.session.config.sid)+"-"+signal.Label() + "]: False")
